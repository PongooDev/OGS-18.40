#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: Prop_Sign_Stop

#include "Basic.hpp"

#include "Parent_BuildingPropActor_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass Prop_Sign_Stop.Prop_Sign_Stop_C
// 0x0000 (0x0E20 - 0x0E20)
class AProp_Sign_Stop_C final : public AParent_BuildingPropActor_C
{
public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"Prop_Sign_Stop_C">();
	}
	static class AProp_Sign_Stop_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<AProp_Sign_Stop_C>();
	}
};
static_assert(alignof(AProp_Sign_Stop_C) == 0x000008, "Wrong alignment on AProp_Sign_Stop_C");
static_assert(sizeof(AProp_Sign_Stop_C) == 0x000E20, "Wrong size on AProp_Sign_Stop_C");

}

